package vn.haohan.lunar.api.integration.bridge.placeholder;
...
public final class PlaceholderResolver {
    ...
    public static String resolve(String template, SkillCastContext context, LivingEntity target, VariableManager variableManager) {
        ActiveMob caster = context != null ? context.caster() : null;
        return resolve(template, caster, target, context, variableManager);
    }

    public static String resolve(String template, ActiveMob caster, LivingEntity target, VariableManager variableManager) {
        return resolve(template, caster, target, null, variableManager);
    }

    public static String resolve(String template, ActiveMob caster, LivingEntity target,
    ...
    private static String resolveSinglePass(String input, ActiveMob caster, LivingEntity target,
                                            SkillCastContext context, VariableManager variableManager) {
    ...
    private static String evaluateTag(String rawTag, ActiveMob caster, LivingEntity target,
                                      SkillCastContext context, VariableManager variableManager) {
        ...
        if (lower.startsWith("mob.") || lower.startsWith("caster.")) {
            ...
            return switch (sub) {
                ...
                case "threat.top" -> caster.threatTable().getTopTarget().map(UUID::toString).orElse("none");
                default -> {
        ...
        if (lower.startsWith("global.var.") && variableManager != null) {
            ...
            return variableManager.getGlobal().get(varName).map(VariableValue::asString).orElse("");
        }

        // Cast Variables
        if (lower.startsWith("cast.var.") && context != null) {
            String varName = lower.substring(9);
            return context.castVariables().get(varName).map(VariableValue::asString).orElse("");
        }
    ...
    private static String safeGetMobName(ActiveMob mob) {
    ...
    private static int countNearbyPlayers(ActiveMob caster, double radius) {
    ...
    private static String formatDistance(ActiveMob caster, LivingEntity target) {
